...
m_inf_threshold = 10 # What m value to be considered as "infinity"
...
class VideoAnalyzer(DataAnalyzer):
    ...
    def get_candidate_frames(self, angles_1, analyzeMaxPosition=False):
        candidate_frames = []
        # print(angles_1)
        if len(angles_1) > 0:
            # min_index = 0
    ...
    def add_side_prefix(self, joints):
        ...
        for j in joints:
            # Check if joint already has LEFT_ or RIGHT_ prefix
            if j == "MOUTH":
                # MOUTH doesn't need side prefix
                left_side.append(j + "_LEFT")
                right_side.append(j + "_RIGHT")
            elif j.startswith("LEFT") or j.startswith("RIGHT"):
                left_side.append(j)
                right_side.append(j)
            else:
                # Add LEFT_ and RIGHT_ prefixes
                left_side.append("LEFT_" + j)
                right_side.append("RIGHT_" + j)
        ...
        if assessment_type == "line":
            for joints in analyzed_joints:
                ...
                for joint in joint_pair:
                    ...
                    if slope == 'inf':
                        scores.append(self.get_percent_diff(slope, target_slope, m_inf_threshold))
                    else:
                        scores.append(self.get_percent_diff(abs(slope), target_slope, m_inf_threshold))
                ...
                for joints_2 in second_joints:
                    ...
                    if target_slope != "None":
                        if slope_left_first is not None and slope_left_second is not None:
                            diff_1 = self.get_percent_diff(slope_left_first, target_slope, m_inf_threshold)
                            diff_2 = self.get_percent_diff(slope_left_second, target_slope, m_inf_threshold)
                            scores.append(np.average(np.array([diff_1, diff_2])))
                        if slope_right_first is not None and slope_right_second is not None:
                            diff_1 = self.get_percent_diff(slope_right_first, target_slope, m_inf_threshold)
                            diff_2 = self.get_percent_diff(slope_right_second, target_slope, m_inf_threshold)
                            scores.append(np.average(np.array([diff_1, diff_2])))
                    else:
                        if slope_left_first is not None and slope_left_second is not None:
                            scores.append(self.get_percent_diff(slope_left_first, slope_left_second, m_inf_threshold))
                        if slope_right_first is not None and slope_right_second is not None:
                            scores.append(self.get_percent_diff(slope_right_first, slope_right_second, m_inf_threshold))
